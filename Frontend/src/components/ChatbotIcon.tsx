...
export function ChatbotIcon({ className, innerColor, ...props }: ChatbotIconProps) {
    return (
        ...
        >
            ...
            {/* Inner parts (eyes and mouth) - use provided color or fall back to high-contrast token */}
            <rect x="40" y="48" width="4" height="10" rx="2" fill={innerColor || "currentColor"} className={!innerColor ? "text-primary-foreground" : ""} />
            <rect x="56" y="48" width="4" height="10" rx="2" fill={innerColor || "currentColor"} className={!innerColor ? "text-primary-foreground" : ""} />

            <path d="M42 65C42 65 45 70 50 70C55 70 58 65 58 65" stroke={innerColor || "currentColor"} strokeWidth="4" strokeLinecap="round" className={!innerColor ? "text-primary-foreground" : ""} />
        </svg>
    );
}
